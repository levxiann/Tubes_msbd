...
@section('content')
    ...
    <div id="content" class="p-4 p-md-5 pt-5"> {{-- wajib pakai --}}
        ...
        <form method="POST" action="{{url('/outmail/store')}}" enctype="multipart/form-data">
            @csrf
            <div class="form-group mb-3">
                <label class="label" for="nooutmail">No Surat</label>
                <input id="nooutmail" name="nooutmail" type="text" class="form-control @error('nooutmail') is-invalid @enderror" placeholder="No Surat" value="{{old('nooutmail')}}" autocomplete="name" autofocus>
                @error('nooutmail')
            ...
            <div class="form-group mb-3">
                ...
                <input id="tanggalkeluar" name="tanggalkeluar" type="date" class="form-control @error('tanggalkeluar') is-invalid @enderror" placeholder="Tanggal Keluar" value="{{old('tanggalkeluar')}}" autocomplete="name" autofocus>
                @error('tanggalkeluar')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
